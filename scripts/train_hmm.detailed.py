...
print("\nHMM – 5-fold CV on DETAILED tagset")
...
for fold, (train_idx, test_idx) in enumerate(kf.split(sentences), start=1):
    ...
    print(f"Training on {len(train_sents)} sentences, testing on {len(test_sents)}")

    # Lidstone smoothing reduces zero-probability problems for sparse transitions and emissions
    estimator = lambda fd, bins: LidstoneProbDist(fd, 0.1, bins)

    # Train the HMM only on the current training fold
    trainer = HiddenMarkovModelTrainer()
    tagger = trainer.train_supervised(train_sents, estimator=estimator)
    ...
    for sent in test_sents:
        ...
        pred = [t.strip() for _, t in tagger.tag(words)]
...
print("\nSample qualitative errors (HMM, detailed):")
print(f"{'Fold':<5} {'Word':<15} {'Gold':<20} {'Pred':<20} {'Left':<15} {'Right':<15}")
print("-" * 95)

for e in qual_errors[:20]:
    fold, w, g, p, l, r, _ = e
    print(f"{fold:<5} {w:<15} {g:<20} {p:<20} {l:<15} {r:<15}")

# Save the full error list for qualitative analysis and comparison with Brill
with open("hmm_qual_errors_detailed.tsv", "w", encoding="utf-8") as f:
    f.write("fold\tword\tgold\tpred\tleft_ctx\tright_ctx\tsentence\n")
    for fold, w, g, p, l, r, sent in qual_errors:
        f.write(f"{fold}\t{w}\t{g}\t{p}\t{l}\t{r}\t{sent}\n")

print("\nSaved qualitative errors to: hmm_qual_errors_detailed.tsv")
...
ax.set_title("HMM – Per-tag F1 (5-fold CV, detailed)")
plt.tight_layout()
plt.savefig("hmm_per_tag_f1_detailed_cv.png", dpi=300)
plt.close(fig)

print("\nSaved plot to: hmm_per_tag_f1_detailed_cv.png")